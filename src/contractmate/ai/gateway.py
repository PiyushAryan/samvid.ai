...
class UrllibJSONTransport:
    def post_json(
        ...
        *,
        ...
        try:
            with urlopen(request, timeout=timeout_seconds) as response:  # noqa: S310 - URL comes from trusted configuration.
                body = response.read()
        except HTTPError as exc:
            detail = exc.read().decode("utf-8", errors="replace")
            raise GatewayAPIError(_error_message(detail, fallback=str(exc)), status_code=exc.code) from exc
        except (URLError, TimeoutError) as exc:
            raise GatewayAPIError(f"Vercel AI Gateway request failed: {exc}") from exc

        try:
            decoded = json.loads(body)
        except (TypeError, json.JSONDecodeError) as exc:
            raise GatewayAPIError("Vercel AI Gateway returned an invalid JSON response.") from exc
        if not isinstance(decoded, dict):
            raise GatewayAPIError("Vercel AI Gateway returned an unexpected response shape.")
        return decoded
...
class GatewayEmbeddingsClient:
    """OpenAI-compatible embedding adapter for Vercel AI Gateway."""
    ...
    def __post_init__(self) -> None:
        if not self.api_key.strip() or not self.model_id.strip():
            raise ValueError("AI Gateway API key and embedding model ID are required.")
    ...
    def embed_documents(self, texts: Sequence[str]) -> tuple[EmbeddingVector, ...]:
        ...
        if not isinstance(data, list) or len(data) != len(cleaned):
            raise GatewayAPIError("AI Gateway embeddings response did not match the requested inputs.")

        vectors: list[EmbeddingVector] = []
        for fallback_index, item in enumerate(data):
            if not isinstance(item, Mapping):
                raise GatewayAPIError("AI Gateway embeddings response contained an invalid item.")
            index = item.get("index", fallback_index)
            values = item.get("embedding")
            if not isinstance(index, int) or not isinstance(values, list) or not values:
                raise GatewayAPIError("AI Gateway embeddings response contained an invalid vector.")
            if any(isinstance(value, bool) or not isinstance(value, Real) for value in values):
                raise GatewayAPIError("AI Gateway embeddings response contained a non-numeric vector.")
        ...
        if [vector.index for vector in vectors] != list(range(len(cleaned))):
            raise GatewayAPIError("AI Gateway embeddings response contained invalid indices.")
        dimensions = {len(vector.values) for vector in vectors}
        if len(dimensions) != 1:
            raise GatewayAPIError("AI Gateway embeddings response contained inconsistent dimensions.")
        actual_dimensions = dimensions.pop()
        if actual_dimensions != self.dimensions:
            raise GatewayAPIError(
                "AI Gateway embeddings response dimension mismatch: "
...
class GatewayRerankClient:
    """Reranking adapter for Vercel AI Gateway's unified rerank endpoint."""
    ...
    def __post_init__(self) -> None:
        if not self.api_key.strip() or not self.model_id.strip():
            raise ValueError("AI Gateway API key and reranking model ID are required.")
    ...
    def rerank(self, *, query: str, documents: Sequence[str], top_n: int | None = None) -> tuple[RerankResult, ...]:
        ...
        if not isinstance(raw_results, list):
            raise GatewayAPIError("AI Gateway rerank response did not contain results.")
        results: list[RerankResult] = []
        seen: set[int] = set()
        for item in raw_results:
            if not isinstance(item, Mapping):
                raise GatewayAPIError("AI Gateway rerank response contained an invalid result.")
        ...
        if top_n is not None and len(results) > top_n:
            raise GatewayAPIError("AI Gateway rerank response exceeded the requested top_n.")
